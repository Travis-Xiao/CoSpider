...
(function() {
    ...
    var room, listeners = {
        ...
        /* 进出房间事件 */
        "into-room": function (data){
            var _id = app.currentUser._id;

            if (_id == data.ownerID)
                room.changeMode(true);
            else
                room.changeMode(false);
        },

        /* 更新文本缓存 */
        updateBuffer: function (data){
			//console.log(data);
            room.calcCommentArea(data.buffer);
        },
        ...
        change: function(data) {
            ...
            var startPos = room.view.editor.posFromIndex(tfrom);
            var endPos = room.view.editor.posFromIndex(tto);
			...
			//room.view.editor.setValue(room.docData.text);
            return;
        },
