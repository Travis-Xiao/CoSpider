...
io.sockets.on('connection', function(socket){
    ...
    socket.on('new', function(data){ // path, type
        ...
        var user = socket.session.user;
        docDAO.createDoc(user._id, data.path, data.type, function(err, ctime, docId){
			if (!err && docId != null && ctime != null){
				docDAO.createDocRecord(user._id, docId, getCurrenTime(), function (err){
					if (err)
    ...
    socket.on('share', function(data){ // path, name
        ...
        docDAO.addMember(user._id, data.path, data.name, function(err, doc){
			...
			userDAO.getUserByName(data.name, function(err, toUser){
				if(!err){
					docDAO.shareDocRecord(user._id, toUser._id, doc._id, getCurrenTime(), function (err){
						if(!err && rooms[data.path]){
							var room = rooms[data.path];
...
//		}
    ...
    socket.on('into-room', function() {
        ...
        docDAO.findOwner(room.id, function (err, result){
            if (err)
                socket.emit('into-room', err);
            else
                socket.emit('into-room', {ownerID : result.owner});
        });
    });
    ...
    socket.on('updateBuffer', function(){
		...
		socket.emit('updateBuffer', {buffer : room.buffer.toString()});
    });
    ...
    socket.on('saveStatistics', function(data){
		...
		docDAO.updateRecord(user._id, docId, modifyTime, result, function (err){
			if (err)
	...
	socket.on('changeOwner', function (data){
		...
		docDAO.changeOwner(user._id, data.path, data.name, function(err, doc){
			...
			userDAO.getUserByName(data.name, function(err, toUser){
				if (!err){
					docDAO.transferDocRecord(user._id, toUser._id, doc._id, getCurrenTime(), function (err){
						if(!err && rooms[data.path]){
							var room = rooms[data.path];
							socket.broadcast.to(room.id).emit('unshared', {name:data.name, time:new Date().getTime()});
						}
	...
	socket.on('getTimelineData', function(){
		...
		docDAO.findOwner(room.id, function (err, myDoc){
			...
			docDAO.getTimelineData(myDoc, function (err, result){
				if (err)
					return socket.emit('getTimelineData', err);
				else
